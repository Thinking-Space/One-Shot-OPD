...
# ----------------------------------------------------------------------------
# 代码评测路由: 按 data_source 分流到 code_eval (LCB / HumanEval+ / MBPP+)
# 兼容老调用方式: 数学 data_source 走原 compute_score, 代码 data_source 走 code_eval
# ----------------------------------------------------------------------------
_CODE_DATA_SOURCES = {
    # LiveCodeBench
    "livecodebench",
...
# Function-Calling 数据 (xlam 训练 + BFCL 验证)
_FC_DATA_SOURCES = {
    "xlam",
    "bfcl_simple",
    "bfcl_multiple",
...
def reward_func(
    ...
    """统一 reward 入口, 按 data_source 分流:

    - 代码数据 (LCB / HumanEval+ / MBPP+) → verl.utils.reward_score.code_eval.compute_score
    - Function-Calling 数据 (xlam / BFCL) → verl.utils.reward_score.fc_eval.compute_score
    - 其他 (默认数学) → ttrl_math.compute_score
    """
    try:
        if data_source in _CODE_DATA_SOURCES:
            from verl.utils.reward_score.code_eval import compute_score as code_compute_score
