...
#
# This module aggregates math answer-normalization and symbolic-equality
# helpers that share provenance with the sibling modules in this package:
#   - `_strip_string` and the answer normalizers: see `math_normalize.py`
#     (PRIME team, derived in turn from the Hendrycks MATH release)
#   - `symbolic_equal` and the sympy-based graders: see `grader.py`
#     (NVIDIA CORPORATION)
#
...
# These used to be SIGALRM helpers defined here, which broke once verl 0.9.0
# started scoring on a pool thread. See verl/utils/reward_score/_timeout.py.
from .._timeout import run_with_timeout, timeout_ours  # noqa: F401  (re-exported)
...
def _latex_equal_inner(given_answer: str, ground_truth: str) -> bool:
    # Taken out of the `with timeout(1)` block it used to sit in so the limit can
    # be applied to a callable: a block cannot be handed to a helper thread, and
    # off the main thread that is the only way left to enforce one.
    try:
...
def is_latex_equal(given_answer: str, ground_truth: str) -> bool:
    try:
        return run_with_timeout(_latex_equal_inner, 1, given_answer, ground_truth)
    except TimeoutError:
        return False
