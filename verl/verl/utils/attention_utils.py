...
def _get_attention_functions() -> tuple[Callable, Callable, Callable, Callable]:
    ...
    if is_torch_npu_available(check_device=False):
        ...
        try:
            ...
            # bert_padding is only tensor gather/scatter. The NPU copy is the
            # same functions; use it when flash-attn is not installed so
            # remove-padding (trainer old_log_prob) can run under sdpa.
            from verl.utils.npu_flash_attn_utils import index_first_axis, pad_input, rearrange, unpad_input
