...
from verl.trainer.ppo.opd_trainer_mixin import OPDTrainerMixin, opd_teacher_enabled, template_enabled
...
class RayPPOTrainer(OPDTrainerMixin):
    ...
    def __init__(
        ...
        # OPD: an independent FSDP teacher scoring the student's own rollout.
        # Disjoint from `use_teacher_policy` above, which drives upstream's
        # inference-replica teacher loop; the two are mutually exclusive.
        self.use_opd_teacher = opd_teacher_enabled(self.config)
        self.use_template_mode = template_enabled(self.config)
        self.validate_opd_top_k_config()
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        # An empty `data.val_files` means "this run has no in-training validation".
        # Stock 0.9.0 cannot express that: it builds the val dataloader
        # unconditionally and asserts it is non-empty, so a domain whose benchmark
        # the single-turn val loop cannot run (IF, scored only by offline Multi-IF)
        # had no way to start. verl's own SFT trainer
        # already carries the equivalent `val_dataloader = None` branch.
        self.validation_enabled = val_dataset is not None or bool(self.config.data.val_files)
        if val_dataset is None and self.validation_enabled:
        ...
        if self.validation_enabled:
            ...
            # Not None: `for test_data in self.val_dataloader` and len() both stay
            # valid, so nothing downstream has to special-case the type.
            self.val_dataloader = []

        assert len(self.train_dataloader) >= 1, "Train dataloader is empty!"
        if self.validation_enabled:
            # Still an error when val_files was given and turned out to hold no
            # usable rows -- e.g. data.filter_overlong_prompts dropped every one.
            assert len(self.val_dataloader) >= 1, "Validation dataloader is empty!"

        print(
            f"Size of train dataloader: {len(self.train_dataloader)}, Size of val dataloader: "
            f"{len(self.val_dataloader)}"
            + ("" if self.validation_enabled else " (validation disabled: data.val_files is empty)")
        )
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            # flatten first so OPD top-K's trailing candidate axis collapses into the
            # per-sequence total instead of leaving one list per position.
            scores = batch.batch["token_level_scores"].flatten(start_dim=1).sum(-1).cpu().tolist()
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        # Keep `ability` on the leftover batch: generate_sequences may drop
        # extra non-tensor columns, and MOPD routing reads this field after
        # the gen-output union.
        reward_keys = set({"data_source", "reward_model", "extra_info", "uid", "ability"}) & batch.non_tensor_batch.keys()
    ...
    def _validate(self, merged: bool = False):
        # Defensive: fit() already gates its two call sites, but _validate is a
        # public-ish hook and the tail of this method (np.concatenate over the
        # per-batch lists) raises on an empty val set rather than returning empty.
        if not getattr(self, "validation_enabled", True):
            return {} if not merged else None
    ...
    def init_workers(self):
        ...
        # OPD: colocate the frozen teacher(s) with the actor.
        if self.use_opd_teacher:
            self.register_opd_teacher(actor_rollout_resource_pool)
        ...
        # Load teacher weights after the actor is up, so vLLM's KV-cache
        # estimate is made against the actor's footprint alone.
        if self.use_opd_teacher:
            self.init_opd_teacher(all_wg)
    ...
    def _compute_old_log_prob(self, batch: DataProto):
        ...
        # OPD: ask the engine for the student's top-K support set in the same
        # forward pass that produces old_log_probs (see opd_student_top_k()).
        opd_top_k = self.opd_student_top_k()
        if opd_top_k > 0:
            tu.assign_non_tensor(batch_td, opd_student_top_k=opd_top_k)
        output = self.actor_rollout_wg.compute_log_prob(batch_td)
        # gather output
        ...
        if opd_top_k > 0:
            # (bsz, max_response_len, K) after the same left-shift-by-one that
            # no_padding_2_padding applies to log_probs, so the K-th candidate at
            # position t lines up with old_log_probs[t] and responses[t].
            top_k_ids = no_padding_2_padding(tu.get(output, "student_top_k_ids"), batch_td)
            top_k_log_probs = no_padding_2_padding(tu.get(output, "student_top_k_log_probs"), batch_td)
            result["student_top_k_ids"] = top_k_ids.long()
            result["student_top_k_log_probs"] = top_k_log_probs.float()
    ...
    def _update_actor(self, batch: DataProto) -> DataProto:
        ...
        # OPD: in top-K reward mode the advantage has a trailing candidate axis, so
        # the training forward has to emit the student's candidate log-probs too --
        # ppo_loss takes the policy gradient over them (see opd_top_k_is_reward()).
        if self.opd_top_k_is_reward():
            tu.assign_non_tensor(batch_td, opd_student_top_k=self.opd_student_top_k())
    ...
    def fit(self):
        ...
        if self.config.trainer.get("val_only", False):
            assert self.validation_enabled, (
                "trainer.val_only=True but data.val_files is empty -- there is nothing to score."
            )
        if self.validation_enabled and self.config.trainer.get("val_before_train", True):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # 0.9.0 generate_sequences already fills response_mask, so the
                    # block above is often a no-op. Template filter stats must
                    # still run (L4 greps for `[Template]`).
                    if self.use_template_mode:
                        self.log_template_metrics(batch, metrics)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # OPD: overwrite the sparse verifier score with the
                        # teacher's dense per-token reward. Runs here because it
                        # needs `old_log_probs` (computed above) and must land
                        # before compute_advantage consumes token_level_rewards.
                        if self.use_opd_teacher:
                            with marked_timer("opd_teacher", timing_raw, color="purple"):
                                batch = self.compute_opd_token_reward(batch, metrics)
