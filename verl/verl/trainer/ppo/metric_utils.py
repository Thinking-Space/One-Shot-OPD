...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    """
    # OPD top-K reward mode gives the token-level tensors a trailing candidate axis
    # of size K. Flattening first keeps these per-sequence totals per-sequence; a
    # plain sum(-1) would silently reduce only the candidate axis and leave a
    # (bsz, response_length) tensor that still averages to a finite-looking number.
    # flatten(start_dim=1) is the identity on the 2-D case.
    sequence_score = batch.batch["token_level_scores"].flatten(start_dim=1).sum(-1)
    sequence_reward = batch.batch["token_level_rewards"].flatten(start_dim=1).sum(-1)
    ...
    # Same candidate axis as above: broadcast the (bsz, response_length) mask over it.
    adv_mask = response_mask if advantages.dim() == response_mask.dim() else response_mask.unsqueeze(-1)
    returns_mask = response_mask if returns.dim() == response_mask.dim() else response_mask.unsqueeze(-1)
    valid_adv = torch.masked_select(advantages, adv_mask.expand_as(advantages))
    valid_returns = torch.masked_select(returns, returns_mask.expand_as(returns))
...
def compute_variance_proxy_metrics(batch: DataProto, gradient_norm: float = None) -> dict[str, float]:
    ...
    advantages = batch.batch["advantages"]
    if advantages.dim() == 3:
        # OPD top-K reward mode: collapse the candidate axis the way the top-K
        # policy loss does (sum over K) to recover a per-position advantage.
