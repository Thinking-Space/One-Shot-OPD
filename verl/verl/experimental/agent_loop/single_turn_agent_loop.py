...
class SingleTurnAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], priority: int = 0, **kwargs) -> AgentLoopOutput:
        ...
        # Validation may run at its own budget (rollout.val_kwargs.max_tokens);
        # generate_sequences puts that number in sampling_params, and it has to
        # be the cap here too or vLLM's extra tokens are silently cut back to the
        # training length. Read it before generate() -- the server pops the key.
