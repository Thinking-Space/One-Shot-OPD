...
class AgentData:
    ...
    def __init__(
        ...
        # Response budget for this trajectory, in new tokens. Set by ``run()``
        # from sampling_params, because validation may carry its own budget
        # (rollout.val_kwargs.max_tokens) that differs from the training
        # rollout.response_length. Left None here on purpose: a code path that
        # forgets to set it should raise on the first comparison rather than
        # silently terminate every trajectory after zero tokens.
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # Validation may run at its own budget; generate_sequences puts it in
        # sampling_params["max_tokens"] and the server pops that key, so read it
        # once here and carry it on agent_data -- the turn-budget checks in the
        # state handlers do not see sampling_params.
        agent_data.response_length = int(sampling_params.get("max_tokens") or self.response_length)
        ...
        output: AgentLoopOutput = AgentLoopOutput(
            ...
            response_ids=response_ids[: agent_data.response_length],
            response_mask=agent_data.response_mask[: agent_data.response_length],
            multi_modal_data=multi_modal_data,
            mm_processor_kwargs=agent_data.mm_processor_kwargs,
            response_logprobs=agent_data.response_logprobs[: agent_data.response_length]
            if agent_data.response_logprobs
            else None,
            num_turns=agent_data.user_turns + agent_data.assistant_turns + 1,
            metrics=agent_data.metrics,
            routed_experts=(
                agent_data.routed_experts[: len(prompt_ids) + agent_data.response_length]
    ...
    async def _handle_generating_state(
        ...
        if not ignore_termination and len(agent_data.response_mask) >= agent_data.response_length:
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        if self.enable_continuous_token and not new_images_this_turn:
            ...
            if len(response_mask) >= agent_data.response_length:
                return AgentState.TERMINATED
            agent_data.prompt_ids = merge_result.token_ids
            agent_data.response_mask = response_mask
            if agent_data.response_logprobs:
        ...
        if len(agent_data.response_mask) + len(response_ids) >= agent_data.response_length:
            return AgentState.TERMINATED
