...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Columns every part carries, rather than the first part's columns.
        #
        # A mixed-domain rollout (MOPD) is scored by several verifiers at once
        # and they return different reward_extra_info keys -- the math grader
        # adds extracted_gt and pred, if_eval adds its strict/loose breakdown,
        # code_eval_v2 adds neither. Each AgentLoopWorker builds its columns from
        # the rows it happens to hold, so the parts arriving here disagree, and
        # list_of_dict_to_dict_of_list asserts on the first part that does.
        #
        # A column missing from some parts cannot be concatenated into a
        # full-length array anyway, so the intersection is the most that can be
        # kept. Single-domain runs are unaffected: one verifier, one key set,
        # and the intersection is the whole of it.
        non_tensor_dicts = [d.non_tensor_batch for d in data]
        shared_keys = [k for k in non_tensor_dicts[0] if all(k in d for d in non_tensor_dicts)]
        non_tensor_batch = {k: np.concatenate([d[k] for d in non_tensor_dicts], axis=0) for k in shared_keys}
        ...
        if data:
            ...
            for d in data:
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        # This names the reward columns, so it disagrees between
                        # parts for exactly the reason the columns themselves do,
                        # and asserting equality here is the same bug one level
                        # further on. It is resolved below rather than merged,
                        # because it has to name what actually survived the
                        # intersection -- the reward manager indexes
                        # non_tensor_batch with these keys and would otherwise
                        # ask for a column that is no longer there.
                        reward_key_lists.append(v)
                    else:
            ...
            if reward_key_lists:
                merged_meta_info["reward_extra_keys"] = [
                    k for k in reward_key_lists[0] if all(k in lst for lst in reward_key_lists) and k in non_tensor_batch
                ]
