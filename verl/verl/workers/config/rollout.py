...
class SamplingConfig(BaseConfig):
    ...
    do_sample: bool = True
    n: int = 1
    #: Response budget for this sampling phase, in new tokens. ``None`` means
    #: "use ``rollout.response_length``", which is the stock 0.9.0 behaviour and
    #: the only behaviour there was: validation reused the training rollout
    #: length, so a run that trains at 7168 could not score at 31744 without a
    #: second, separate val-only run. Set on ``rollout.val_kwargs`` it caps
    #: generation during validation only, and the response tensors of that
    #: validation batch are sized to it (see AgentLoopBase.effective_response_length).
    #: Must fit in ``rollout.max_model_len`` -- the vLLM server clamps to
    #: ``max_model_len - len(prompt)`` and would otherwise truncate silently.
