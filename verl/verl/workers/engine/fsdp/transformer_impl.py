...
class FSDPEngineWithLMHead(FSDPEngine):
    ...
    def _emit_opd_student_top_k(self, logits: torch.Tensor, top_k: int) -> dict[str, torch.Tensor]:
        """Extract the student's own top-K token ids and their log-probs.

        OPD needs the student's top-K support set to build the top-K distillation
        reward. It is computed here because this is the only point in the stack
        where the full-vocabulary student logits are materialized; recomputing
        them anywhere else would cost a second forward pass.

        This deliberately does not go through ``logits_processor_func``: that
        seam is bound to the engine's ``loss_fn``, which on the OPD path is the
        actor's PPO loss.

        The log-sum-exp runs over the full vocabulary but only the K gathered
        entries are normalized, so peak extra memory is (tokens, K) rather than
        a second (tokens, vocab) tensor.
        """
        top_k = min(top_k, logits.shape[-1])
        top_k_logits, top_k_ids = torch.topk(logits, k=top_k, dim=-1)
        log_probs = top_k_logits.float() - torch.logsumexp(logits.float(), dim=-1, keepdim=True)
        return {
            "student_top_k_ids": top_k_ids.to(torch.int64),
            "student_top_k_log_probs": log_probs,
        }
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        opd_student_top_k = tu.get_non_tensor_data(data=micro_batch, key="opd_student_top_k", default=0)
        ...
        if opd_student_top_k > 0 and use_fused_kernels:
            raise NotImplementedError(
                "OPD top-K distillation requires actor_rollout_ref.model.use_fused_kernels=False: "
                "fused kernels never materialize the full logits tensor, so the student's top-K "
                "support set cannot be extracted. Set log_prob_top_k=0/metric_top_k=0 to disable "
                "top-K, or disable fused kernels."
            )
        ...
        if use_remove_padding:
            ...
            if use_fused_kernels:
                ...
                if opd_student_top_k > 0:
                    opd_top_k_rmpad = self._emit_opd_student_top_k(logits_rmpad, opd_student_top_k)
            ...
            if opd_student_top_k > 0 and not use_fused_kernels:
                # (total_nnz, K): gather_dim/unpad_dim are both the token axis, so
                # the trailing K axis rides along untouched.
                opd_top_k_rmpad = {
                    k: self._gather_and_unpad_packed(v, pad_size) for k, v in opd_top_k_rmpad.items()
                }
            ...
            if pad_mode == DatasetPadMode.NO_PADDING:
                ...
                if opd_student_top_k > 0 and not use_fused_kernels:
                    for k, v in opd_top_k_rmpad.items():
                        model_output[k] = torch.nested.nested_tensor_from_jagged(v, cu_seqlens)
            else:
                raise NotImplementedError(f"pad_mode {pad_mode} not implemented")
            ...
            if use_fused_kernels:
                ...
                if pad_mode == DatasetPadMode.NO_PADDING:
                    ...
                    if opd_student_top_k > 0:
                        for k, v in self._emit_opd_student_top_k(logits_rmpad, opd_student_top_k).items():
                            model_output[k] = torch.nested.nested_tensor_from_jagged(v, cu_seqlens)
