...
def ppo_loss(config: ActorConfig, model_output, data: TensorDict, dp_group=None):
    ...
    # OPD top-K reward mode: the teacher reward, and hence the advantage, carries a
    # trailing candidate axis of size K. The policy gradient must then be taken over
    # the student's log-probs on those same K candidates instead of on the single
    # sampled token, so both sides of the importance ratio move to the (bs, T, K)
    # axis. Metric-only top-K never reaches here: it leaves the reward 2-D and the
    # engine is not asked to emit the candidate log-probs.
    top_k_log_probs = model_output.get("student_top_k_log_probs", None)
    if top_k_log_probs is not None:
        log_prob = no_padding_2_padding(top_k_log_probs, data)
    ...
    if top_k_log_probs is not None:
        fields.append("student_top_k_log_probs")
    ...
    response_mask = data["response_mask"].to(bool)
    # compute policy loss
    old_log_prob = data["student_top_k_log_probs"] if top_k_log_probs is not None else data["old_log_probs"]
    ...
    if config.use_kl_loss:
        if top_k_log_probs is not None:
            raise NotImplementedError(
                "actor.use_kl_loss=True is not supported with OPD top-K reward mode: the reference "
                "log-probs are (bs, T) over sampled tokens while the student's are (bs, T, K) over "
                "candidates, so the two do not describe the same distribution. Set use_kl_loss=False, "
                "or use metric_top_k instead of log_prob_top_k."
            )
